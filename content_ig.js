...
(function () {
  ...
  // Read handle — works on /handle/, /handle/reels/, /handle/tagged/, etc.
  function getHandle() {
    const segs = location.pathname.split('/').filter(Boolean);
    const handle = segs[0];
    const skip = ['explore', 'reel', 'p', 'stories', 'direct', 'accounts', 'reels', 'tv'];
    if (handle && !skip.includes(handle)) return handle;
    return null;
  }
  ...
  function ingest(data) {
    try {
      ...
      // ── Direct top-level clips connection (some API variants) ─
      const directConn =
  ...
  function parseGQL(n) {
    ...
    // Support both GQL __typename and V1 media_type
  ...
  window.__vs_ig_sortGrid = function (ids) {
    ...
    Object.keys(map).forEach(id => { if (!ids.includes(id)) map[id].style.order = '9999'; });
    return true;
  };
